...
function toast(msg){
  ...
  window.__t = setTimeout(() => $("toast").style.display = "none", 3500);
}
...
function subscribeTenantData(tid){
  ...
  unsubAccounts = onSnapshot(accQ, (ss) => {
  ...
  unsubExpenses = onSnapshot(expQ, (ss) => {
...
onAuthStateChanged(auth, async (u) => {
  ...
  if (!profile || !profile.tenantId){
    // No autorizado (no tiene perfil)
    unsubscribeAll();
    if ($("loginScreen")) $("loginScreen").style.display = "flex";
    if ($("loginError")) $("loginError").textContent = "Este usuario no está autorizado (falta users/{uid}).";
    await signOut(auth);
    return;
  }
...
$("btnToggleKpis")?.addEventListener("click", () => { uiToggles.kpis=!uiToggles.kpis; applyToggles(); });
$("btnToggleCharts")?.addEventListener("click", () => { uiToggles.charts=!uiToggles.charts; applyToggles(); });
$("btnToggleTables")?.addEventListener("click", () => { uiToggles.tables=!uiToggles.tables; applyToggles(); });
applyToggles();
...
function openAccModal(title){
  $("modalAccTitle").textContent = title;
  $("modalAccBack").style.display = "flex";
  syncCategoryUI();
}
function closeAccModal(){
  $("modalAccBack").style.display = "none";
  editingAccId = null;
}
$("btnAccClose")?.addEventListener("click", closeAccModal);
$("modalAccBack")?.addEventListener("click", (e) => { if (e.target.id === "modalAccBack") closeAccModal(); });
...
function openExpModal(title){
  ...
  $("modalExpBack").style.display = "flex";
}
function closeExpModal(){
  $("modalExpBack").style.display = "none";
  editingExpId = null;
}
$("btnExpClose")?.addEventListener("click", closeExpModal);
$("modalExpBack")?.addEventListener("click", (e) => { if (e.target.id === "modalExpBack") closeExpModal(); });
...
$("btnAccSave")?.addEventListener("click", async () => {
  ...
  try{
    ...
    toast("Error guardando cuenta (Firestore).");
  }
});
...
$("btnExpSave")?.addEventListener("click", async () => {
  ...
  try{
    ...
    toast("Error guardando gasto (Firestore).");
  }
});
...
function openWhatsApp(a){
  ...
  if (!phone){ toast("No hay celular válido para WhatsApp."); return; }
  ...
  tbody.querySelectorAll("button[data-act]").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      if (act === "edit"){
        ...
        if (!row.expire){ toast("Esta cuenta no tiene expiración."); return; }
...
function renderExpenses(){
  ...
  $("countChipExp").textContent = `${list.length} gastos`;
...
function renderDashboard(){
  ...
  $("countChipDash").textContent = `${db.accounts.length} cuentas · ${db.expenses.length} gastos`;
  $("miniAccounts").textContent = fmt0(db.accounts.length);
  $("miniExpenses").textContent = fmt0(db.expenses.length);
  $("miniCollect").textContent = fmt0(db.accounts.filter(a=>a.collect==="YES").length);
  $("miniUrgent").textContent = fmt0(expState.urgent);
...
function renderAll(){
  // si todavía no hay DOM listo (en algunos casos), no revientes
  if (!$("tbodyAcc") || !$("tbodyExp")) return;
  renderDashboard();
  renderAccounts();
  renderExpenses();
  renderAnalytics();
